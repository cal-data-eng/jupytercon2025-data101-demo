...
# Setup required for old JupySQL setup
...
class GradingUtil(object):
    ...
    def prepare_autograder(self, db_name, queries_dir=QUERIES_DIR):
        ...
        """
        # Create results dir (required for old JupySQL setup)
    ...
    def prepare_submission_and_cleanup(self):
        ...
        command = ["zip",
                   "-r", f"{self.queries_dir}.zip",
                   self.queries_dir]
        results = run(command, stdout=PIPE, stderr=PIPE)
        if results.stderr:
            raise RuntimeError(results.stderr)
        print(f'Created {self.queries_dir}.zip')

        # Below is only required for old JupySQL setup
        ...
        if results.stderr:
            raise RuntimeError(results.stderr)
        print(f'Created {RESULTS_DIR}.zip')
        # End old setup

    ### BEGIN OLD JUPYSQL SETUP METHODS ###
